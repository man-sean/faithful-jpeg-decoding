...
def create_lmdb_for_imagenet():
    ...
    lmdb_path = '/home/sean.man/datasets/imagenet_patch_128/train_mp.lmdb'
...
def prepare_keys_imagenet(folder_path):
    ...
    for subdir in tqdm.tqdm(subdirs, file=sys.stdout):
        img_path_list += list(glob.glob(os.path.join(subdir, '*.JPEG')))
    ...
    keys = [img_path.split('.JPEG')[0] for img_path in img_path_list]
